...
export default function LeadForm({ className = "" }: { className?: string }) {
    ...
    const inputClasses = "w-full px-4 py-3.5 bg-slate-50/80 border border-gray-100/60 rounded-md focus:bg-white focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-300/50 transition-all duration-300 text-[#0A1628] placeholder:text-gray-400/80 font-medium";
    const labelClasses = "text-sm font-semibold text-[#0A1628] block mb-2 tracking-wide";

    if (submitted) {
        return (
            <div className={`text-center py-12 ${className}`}>
                <div className="w-20 h-20 bg-gradient-to-br from-green-50 to-emerald-100 border border-green-100 rounded-lg flex items-center justify-center mx-auto mb-6">
    ...
    return (
        <form onSubmit={handleSubmit} className={`space-y-5 ${className}`}>
            ...
            <button
                type="submit"
                disabled={loading}
                className="w-full py-4 bg-gradient-to-r from-amber-500 via-orange-500 to-amber-500 bg-[length:200%] hover:bg-right text-white font-bold rounded-md focus:outline-none focus:ring-2 focus:ring-amber-400 focus:ring-offset-2 transition-all duration-500 disabled:opacity-50 shadow-[0_8px_30px_rgba(245,158,11,0.3)] hover:shadow-[0_12px_40px_rgba(245,158,11,0.4)] flex items-center justify-center gap-2 text-lg"
            >
                {loading ? (
                    "Sending..."
                ) : (
