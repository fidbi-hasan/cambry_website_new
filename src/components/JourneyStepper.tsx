...
function StepCard({ step, index }: { step: (typeof steps)[number]; index: number }) {
    return (
        <StaggerItem key={step.title} className="group">
            <div className="h-full p-5 rounded-xl border border-white/70 bg-white/80 backdrop-blur-sm shadow-[0_8px_28px_rgba(15,23,42,0.06)] hover:shadow-[0_24px_60px_rgba(15,23,42,0.12)] hover:-translate-y-2 transition-all duration-500 text-center">
                <div className="relative w-22 h-22 rounded-lg bg-gradient-to-br from-slate-50 to-white border border-gray-100 shadow-[0_6px_24px_rgba(0,0,0,0.06)] flex items-center justify-center mb-6 mx-auto">
                    <div className={`absolute inset-0 bg-gradient-to-br ${step.color} opacity-0 rounded-lg group-hover:opacity-100 transition-opacity duration-500`} />
                    <step.icon className="w-8 h-8 text-[var(--brand-blue)] group-hover:text-white relative z-10 transition-colors duration-500" />
                    <div className={`absolute -top-3 -right-3 w-8 h-8 rounded-md bg-gradient-to-br ${step.color} text-white flex items-center justify-center font-extrabold text-sm shadow-lg`}>
                        {index}
                    </div>
                </div>
            ...
            <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                <FadeIn direction="up">
                    <div className="text-center mb-22">
                        ...
                        <h2 className="text-3xl sm:text-4xl lg:text-6xl font-extrabold text-[#0A1628] tracking-tight leading-[1.1]">
