...
export default function Chatbot() {
    ...
    return (
        <div className="fixed bottom-24 right-5 z-50 flex flex-col items-end">
            {/* Chat Window */}
            {isOpen && (
                <div className="w-80 sm:w-96 bg-white rounded-lg shadow-2xl border border-gray-200 overflow-hidden flex flex-col mb-4 animate-fade-up origin-bottom-right" style={{ height: '500px', maxHeight: '70vh' }}>
                    {/* Header */}
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50">
                        {messages.map((msg, i) => (
                            <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                <div className={`max-w-[80%] p-3 rounded-lg ${msg.role === 'user' ? 'bg-[var(--brand-yellow)] text-white rounded-tr-sm' : 'bg-white border border-gray-100 shadow-sm text-gray-800 rounded-tl-sm'}`}>
                        ...
                        {isLoading && (
                            <div className="flex justify-start">
                                <div className="bg-white border border-gray-100 shadow-sm p-3 rounded-lg rounded-tl-sm flex items-center gap-2">
            ...
            {!isOpen && (
                ...
                >
                    ...
                    {/* Tooltip */}
                    <div className="absolute right-full mr-4 bg-white text-gray-800 text-sm px-4 py-2 rounded-md shadow-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap hidden sm:block">
